...
if(!s.includes('function trimTrailingOverviewGaps(values, currentMs)')||!s.includes('vals = trimTrailingOverviewGaps(vals, ms);')) throw new Error('overview trailing gap trim missing');
if(!s.includes('const initialPage = page || "overview";')||!s.includes('loadSeries(index, targetKey || undefined),')||!s.includes('loadSeries(index, "all")')||!s.includes('renderWindow(current.node, current.page || initialPage);')) throw new Error('initial node-detail series rerender missing');
...
if(!n.includes('function alignSeriesToCurrentPing(uuid, byTask)')||!n.includes('alignSeriesToCurrentPing(uuid, byTask);')||!n.includes('tail.is_loss === true')) throw new Error('detail Ping live-tail alignment missing');
