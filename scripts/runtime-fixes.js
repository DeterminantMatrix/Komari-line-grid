...
function fixApp(input) {
  ...
  source = replaceAllRequired(
    source,
    'pingTips(vals, range === "24h" ? 30 : range === "6h" ? 10 : 5)',
    'pingTips(vals, rangeStepMinutes(vals))',
    'range-aware latency tips vals',
    2
  );
  source = replaceAllRequired(
    source,
    'pingTips(ctx.sparkVals, range === "24h" ? 30 : range === "6h" ? 10 : 5)',
    'pingTips(ctx.sparkVals, rangeStepMinutes(ctx.sparkVals))',
    'range-aware latency tips context',
    2
  );
  ...
  source = replaceRequired(
    source,
    '      chart = ProbeCharts.spark(vals, { w: 960, h: 200, color: pingColor(sparkSrc, sparkIndex), fillOpacity: 0.11, tips: pingTips(vals, rangeStepMinutes(vals)) });\n',
  ...
  source = replaceRequired(
    source,
    'placeholder="VPS / 地区 / ASN / 回程"',
    'placeholder="VPS / 地区 / ASN / 服务商"',
    'search placeholder after Return removal'
  );
