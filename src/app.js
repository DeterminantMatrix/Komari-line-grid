...
(function () {
  ...
  function openNode(index, page) {
    ...
    range = "1h";
    const initialPage = page || "overview";
    go(viewHash(route().view || lastView, index, initialPage));
    Promise.all([
      loadSeries(index, targetKey || undefined),
      loadSeries(index, "all")
    ]).then(function () {
      const current = route();
      if (current.node == null || String(current.node) !== String(index)) return;
      renderWindow(current.node, current.page || initialPage);
    });
  }
